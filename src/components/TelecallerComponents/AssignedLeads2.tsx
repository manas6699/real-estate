                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-[9999] p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6">
                        <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800">
                            <History className="text-blue-500" /> Lead Update History
                        </h2>
                            ...
                            {history.slice().reverse().map((item, idx) => (
                                <div key={idx} className="pl-4 border-l-2 border-blue-200 py-1 text-sm relative">
                                    ...
                                    <p className="text-[10px] text-gray-400 uppercase font-bold">
                                        {typeof item === 'string' ? 'System' : new Date(item.updatedAt).toLocaleString('en-GB')}
                                    </p>
                                    <p className="text-gray-700 leading-relaxed">
                                        {typeof item === 'string' ? item : (
                                            <span>
                                                <span className="font-semibold text-gray-900">{item.assignee_name}</span> marked as <span className="font-bold text-blue-600">{item.status}</span>
                                            </span>
                                        )}
                                    </p>
                                    {typeof item !== 'string' && item.remarks && (
                                        <p className="text-xs text-gray-500 italic mt-1 bg-gray-50 p-2 rounded">&quot;{item.remarks}&quot;</p>
                                    )}
                                </div>
                            ))}
                        </div>
                        <button onClick={() => setShowHistory(false)} className="w-full mt-6 py-2.5 bg-gray-900 hover:bg-black text-white rounded-xl font-bold transition-all shadow-lg active:scale-95 cursor-pointer">
                            Close History
                        </button>
                    </div>
                </div>
