...
import { REASSIGN_API, GET_ALL_TELECALLERS_API, GET_ALL_SALES_PERSONS_API } from '@/config/api';
...
export default function ReassignModal({ onClose, leadId }: AssignModalProps) {
    ...
    const fetchUsers = React.useCallback(async () => {
        try {
            ...
            console.log(`Fetched ${activeTab}s:`, response.data.data);
    ...
    const handleSubmit = async () => {
        if (!selectedUser) {
            alert('Please select someone to reassign.');
            return;
        }

        setSubmitLoading(true);

        const historyMessage = `${leadId} has been reassigned to ${selectedUser.name} (${selectedUser.role}) at ${new Date().toISOString()} with remarks: "${remarks}"`;

        try {
            const response = await axios.post(REASSIGN_API, {
                lead_id: leadId,
                assignee_id: selectedUser.id,
                assignee_name: selectedUser.name,
                remarks,
                history_entry: historyMessage, // ✅ Correct key
            });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl w-full max-w-md p-6 relative">
                ...
                <h2 className="text-xl font-bold mb-4">Reassign this Lead to someone else</h2>
                    ...
                    <ul className="space-y-2">
                        {users.map((user) => (
                            ...
                            >
                                ...
                                >
                                    {assigningId === user.id ? 'Selected' : 'Reassign'}
                                </button>
                            </li>
                        ))}
                    </ul>
